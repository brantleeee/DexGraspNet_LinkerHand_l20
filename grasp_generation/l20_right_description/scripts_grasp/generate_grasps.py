...
def generate(args_list):
    ...
    hand_model = HandModel(
        urdf_path='l6_right_description/linkerhand_l6_right.urdf', # 👈 指向你的 L6 URDF
    ...
    # =========================================================================
    # 👑 L6 Right 专属补丁：动态中心 + 坐标系死锁 (大规模生成版)
    # =========================================================================
    with torch.no_grad():
        new_pose = hand_model.hand_pose.detach().clone()
        original_rot_mat = robust_compute_rotation_matrix_from_ortho6d(new_pose[:, 3:9])
        T_old = new_pose[:, 0:3].clone()

        # 1. 动态提取掌心偏移
        palm_contacts = hand_model.mesh['hand_base_link']['contact_candidates']
        dynamic_center = palm_contacts.mean(dim=0) if palm_contacts.shape[0] > 0 else torch.zeros(3, device=device)
        joint_offset_local = dynamic_center.unsqueeze(0).unsqueeze(2).expand(new_pose.shape[0], 3, 1)

        # 2. 锁死掌心世界坐标
        center_global = T_old.unsqueeze(2) + torch.bmm(original_rot_mat, joint_offset_local)

        # 3. 施加旋转修正 (使用你之前调好的角度，例如 0, 0, 0 或 -pi/2, 0, 0)
        correction_mat = torch.tensor(
            transforms3d.euler.euler2mat(0, 0, 0, axes='sxyz'), # 👈 确保这里是你调通的角度
            dtype=torch.float, device=device
        ).unsqueeze(0).expand(new_pose.shape[0], 3, 3)

        new_rot_mat = torch.matmul(original_rot_mat, correction_mat)
        new_pose[:, 3:9] = new_rot_mat[:, :, :2].transpose(1, 2).reshape(-1, 6)

        # 4. 反推手腕位置
        T_new = center_global - torch.bmm(new_rot_mat, joint_offset_local)
        new_pose[:, 0:3] = T_new.squeeze(2)
