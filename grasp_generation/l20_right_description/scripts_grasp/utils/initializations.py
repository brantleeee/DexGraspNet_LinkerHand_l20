...
def initialize_convex_hull(hand_model, object_model, args):
    ...
    # ========================================================
    # 👑 核心修复：告别硬编码！动态自适应任意关节数量的灵巧手
    # ========================================================
    # 动态生成长度为 n_dofs 的全 0 数组，作为初始均值 (代表手指全张开状态)
    joint_angles_mu = torch.zeros(hand_model.n_dofs, dtype=torch.float, device=device)
    
    # 根据手模型的关节上下限，动态计算每个关节合法的初始扰动方差
    ...
    return hand_pose, contact_point_indices