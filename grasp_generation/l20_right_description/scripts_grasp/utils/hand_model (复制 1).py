...
class HandModel:
   ...
   #     num_samples[list(num_samples.keys())[0]] += n_surface_points - sum(num_samples.values())
        ...
        if armor_path is not None and os.path.exists(armor_path):
            ...
            for link_name in self.mesh:
                if link_name in armor_data and len(armor_data[link_name]) > 0:
                    pts = torch.tensor(armor_data[link_name], dtype=torch.float, device=device)
                    # 💡 核心：把离线的完美装甲点，同时赋值给穿模检测和表面距离检测！
                    self.mesh[link_name]['penetration_points'] = pts
                    self.mesh[link_name]['surface_points'] = pts
                else:
                    empty_pts = torch.empty((0, 3), dtype=torch.float, device=device)
                    self.mesh[link_name]['penetration_points'] = empty_pts
                    self.mesh[link_name]['surface_points'] = empty_pts
                    
            print("\n" + "="*50)
            print(f"🛡️ 物理边界统一：已使用离线装甲全面取代在线粗糙采样点！")
            print("="*50 + "\n")
        else:
            print(f"⚠️ 警告：未加载外部穿透点数据 (Path: {armor_path})，可能导致物理引擎异常。")
            for link_name in self.mesh:
                empty_pts = torch.empty((0, 3), dtype=torch.float, device=device)
                self.mesh[link_name]['penetration_points'] = empty_pts
                self.mesh[link_name]['surface_points'] = empty_pts
    ...
    def get_surface_points(self):
        """
        由于 __init__ 中已经统一了点云，这里直接读取 surface_points 即可。
        它们已经是我们精心切割过的完美装甲点了！
        """
        points = []
        batch_size = self.global_translation.shape[0]
