...
R_linker_fix_cpu = get_linker_fix_matrix(args.linker_z_fix_deg)
...
for i in range(len(args.object_code_list)):
    ...
    for j in range(args.batch_size):
        ...
        # qpos 是 LinkerHand L20 真实 body-frame 执行姿态。
        ...
        # Debug-only: Shadow-compatible semantic frame.
        # 不用于执行，只用于可视化/对齐分析。
        rot_shadow_semantic = rot @ R_linker_fix_cpu.T
        euler_shadow_semantic = transforms3d.euler.mat2euler(
            rot_shadow_semantic,
            axes='sxyz'
        )

        qpos_shadow_semantic = dict(zip(joint_names, hand_pose[9:].tolist()))
        qpos_shadow_semantic.update(dict(zip(rot_names, euler_shadow_semantic)))
        qpos_shadow_semantic.update(dict(zip(translation_names, hand_pose[:3].tolist())))
        ...
        rot_st_shadow_semantic = rot_st @ R_linker_fix_cpu.T
        euler_st_shadow_semantic = transforms3d.euler.mat2euler(
            rot_st_shadow_semantic,
            axes='sxyz'
        )

        qpos_st_shadow_semantic = dict(zip(joint_names, hand_pose_init[9:].tolist()))
        qpos_st_shadow_semantic.update(dict(zip(rot_names, euler_st_shadow_semantic)))
        qpos_st_shadow_semantic.update(dict(zip(translation_names, hand_pose_init[:3].tolist())))
