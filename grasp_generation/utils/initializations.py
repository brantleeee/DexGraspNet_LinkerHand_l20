...
# =========================================================================
        ...
        # 如果 L20 掌心不在正中心，还可以微调 X 和 Y，比如 [0.01, 0.0, 0.03]
...
# =========================================================================
    # 💡 关节角度初始化修改：默认设置为零，但 thumb_cmc_roll 设置为 90 度
    # =========================================================================
    
    # 1. 将所有关节初始均值设为 0
    joint_angles_mu = torch.zeros_like(hand_model.joints_lower)
    
    # 🔥 核心修改：单独设置 thumb_cmc_pitch 绕其轴旋转 90 度 (pi / 2)
    if 'thumb_cmc_roll' in hand_model.joints_names:
        thumb_roll_idx = hand_model.joints_names.index('thumb_cmc_roll')
        joint_angles_mu[thumb_roll_idx] = math.pi / 2
    ...
    for i in range(hand_model.n_dofs):
        mean_val = joint_angles_mu[i].item()
        
        # 安全补丁：动态扩展采样上下限，确保均值永远处于合法区间内
        a_limit = min((hand_model.joints_lower[i] - 1e-6).item(), mean_val - 1e-6)
        b_limit = max((hand_model.joints_upper[i] + 1e-6).item(), mean_val + 1e-6)

        # 使用截断正态分布采样
        torch.nn.init.trunc_normal_(
            joint_angles[:, i], 
            mean=mean_val, 
            std=joint_angles_sigma[i].item(), 
            a=a_limit, 
            b=b_limit
        )

    # 3. 拼接位姿：[平移(3), 旋转(6), 关节角(n_dofs)]
    hand_pose = torch.cat([
        translation,
