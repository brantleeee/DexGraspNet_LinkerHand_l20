...
# =========================================================================
    ...
    # 2. 计算标准差（如果希望严格为0，请确保运行参数 --jitter_strength 为 0）
    ...
    for i in range(hand_model.n_dofs):
        # 使用截断正态分布采样
        # 如果 jitter_strength 为 0，采样结果将严格等于 mu (即 0)
        torch.nn.init.trunc_normal_(
            joint_angles[:, i], 
            mean=joint_angles_mu[i].item(), 
            std=joint_angles_sigma[i].item(), 
            a=(hand_model.joints_lower[i] - 1e-6).item(), 
            b=(hand_model.joints_upper[i] + 1e-6).item()
        )
    ...
    # 🔥 核心修复：必须告诉 PyTorch 这个变量是要求导优化的！(你刚才漏掉了这一行)
